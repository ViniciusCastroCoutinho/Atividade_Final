...
class Maze:
    ...
    def mirror_walls_horizontally(self, game_height_start, height):
        ...
        for wall in self.walls:
            if wall[0] == 641:
                ...
                mirrored_wall = pygame.Rect(wall[0], height + game_height_start - wall[1] - wall[3] + 80, wall[2], wall[3])
                temp_walls.append(mirrored_wall)
            elif wall[0] == 199:
                mirrored_wall = pygame.Rect(wall[0], height + game_height_start - wall[1] - wall[3], wall[2], wall[3] + 50)
                temp_walls.append(mirrored_wall)
        self.walls = temp_walls
    ...
    def draw_obstacle(self, screen, background):
        if background == 0:
